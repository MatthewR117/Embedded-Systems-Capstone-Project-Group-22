...
void CState_Machine(void)
{
    switch (CalcState){
        ...
        case CCompute:
        	...
        	{
        		voltage = ((rx_data[3] << 8) | rx_data[4])/10;
        		current = ((rx_data[7] << 8 | rx_data[8]) << 16) | ((rx_data[5] << 8) | rx_data[6]);
        		watts = ((rx_data[11] << 8 | rx_data[12]) << 16) | ((rx_data[9] << 8) | rx_data[10])/10;
        		watthours = ((rx_data[15] << 8 | rx_data[16]) << 16) | ((rx_data[13] << 8) | rx_data[14]);
        	}
        	CalcState = CWait;
        break;
    }
